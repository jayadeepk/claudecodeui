...
function ChatInterface({ selectedProject, selectedSession, ws, sendMessage, messages, onFileOpen, onInputFocusChange, onSessionActive, onSessionInactive, onReplaceTemporarySession, onNavigateToSession, onShowSettings, onClearTodos, onTodoUpdate, autoExpandTools, showRawParameters, autoScrollToBottom, sendByCtrlEnter, permissionMode: externalPermissionMode, onPermissionModeChange, todoPanelOpen, notifyOnComplete, todos }) {
  ...
  // Use external permission mode from props, fallback to internal state for backwards compatibility
  const permissionMode = externalPermissionMode || 'default';
  const setPermissionMode = onPermissionModeChange || (() => {});
  ...
  return (
    <>
      ...
      <div className={`p-2 sm:p-4 md:p-4 flex-shrink-0 ${
        isInputFocused ? 'pb-2 sm:pb-4 md:pb-6' : 'pb-8 sm:pb-4 md:pb-6'
      }`}>
        ...
        {isUserScrolledUp && chatMessages.length > 0 && (
          <button
            ...
            className="absolute left-1/2 transform -translate-x-1/2 bottom-32 w-8 h-8 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg flex items-center justify-center transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:ring-offset-gray-800 z-10"
        ...
        <form onSubmit={handleSubmit} className="relative max-w-4xl mx-auto">
          ...
          <div {...getRootProps()} className={`relative bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-600 focus-within:ring-2 focus-within:ring-blue-500 dark:focus-within:ring-blue-500 focus-within:border-blue-500 transition-all duration-200 ${isTextareaExpanded ? 'chat-input-expanded' : ''}`}>
            ...
            <textarea
              ...
              disabled={isLoading}
              rows={1}
              className="chat-input-placeholder w-full pl-12 pr-28 sm:pr-40 pt-3 pb-0 sm:pt-4 sm:pb-0 bg-transparent rounded-2xl focus:outline-none text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 disabled:opacity-50 resize-none min-h-[40px] sm:min-h-[56px] max-h-[40vh] sm:max-h-[300px] overflow-y-auto text-sm sm:text-base transition-all duration-200"
              style={{ height: 'auto' }}
            />
            ...
            <button
              type="button"
              onClick={open}
              className="absolute left-2 bottom-1 sm:bottom-3 px-1 py-3 sm:px-2 sm:py-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
              title="Attach images"
            >
            ...
            <button
              ...
              className="absolute right-2 top-1/2 transform -translate-y-1/2 w-8 h-8 sm:w-8 sm:h-8 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed rounded-full flex items-center justify-center transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:ring-offset-gray-800"
            >
              <svg 
